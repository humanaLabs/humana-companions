...
export async function PATCH(
  ...
  try {
    ...
    if (!isCurrentUserMasterAdmin) {
      // Admin regular não pode alterar privilégios de Master Admin
      if (isMasterAdmin !== undefined) {
        return NextResponse.json({ 
          error: 'Apenas Master Admin pode alterar privilégios de Master Admin' 
        }, { status: 403 });
      }

      // Admin regular não pode alterar organização
    ...
    console.log('Mock: Atualizando usuário', userId, 'com dados:', {
      name, email, roleId, status, organizationId, isMasterAdmin
    });

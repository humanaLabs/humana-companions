...
async function runTests() {
  ...
  try {
    ...
    console.log(
      '❌ FAILED - Valid session test threw error:',
  ...
  try {
    ...
    if (response.status === 401) {
      ...
      console.log(
        '❌ FAILED - Missing session should return 401, got:',
        response.status,
      );
      failed++;
    }
    ...
    console.log(
      '❌ FAILED - Missing session test threw error:',
      error instanceof Error ? error.message : String(error),
    );
    failed++;
  }
  ...
  try {
    ...
    if (response.status === 403) {
      ...
      console.log(
        '❌ FAILED - Missing organizationId should return 403, got:',
        response.status,
      );
      failed++;
    }
    ...
    console.log(
      '❌ FAILED - Missing organizationId test threw error:',
      error instanceof Error ? error.message : String(error),
    );
    failed++;
  }
  ...
  try {
    ...
    if (response.status === 403) {
      ...
      console.log(
        '❌ FAILED - Invalid organizationId should return 403, got:',
        response.status,
      );
      failed++;
    }
    ...
    console.log(
      '❌ FAILED - Invalid organizationId test threw error:',
      error instanceof Error ? error.message : String(error),
    );
    failed++;
  }
  ...
  try {
    ...
    const path1 = extractOrganizationFromPath(
      '/api/organizations/org-123/companions',
    );
    ...
    console.log(
      '❌ FAILED - Path extraction test threw error:',
      error instanceof Error ? error.message : String(error),
    );
    failed++;
  }
  ...
  try {
    ...
    console.log(
      '❌ FAILED - Organization access test threw error:',
      error instanceof Error ? error.message : String(error),
    );
    failed++;
  }
